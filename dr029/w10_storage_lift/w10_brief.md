# W10 — Storage lift to top-level `store/` (brief)

**Status:** Locked.
**Locked:** 2026-05-11 16:36 ACST (Session 118).
**Output:** `dr029/w10_storage_lift/w10_report.md` (single file, post-execution).
**Anchor precedent:** Sessions 35/36 surgical-fix briefs (named changes in dependency order, hard limits explicit, pre-and-post empirical verification).
**Governing DRs:** DR-030 (v3 repo layout — load-bearing); DR-031 (Alembic locked but deferred); DR-032 (canonical bet-record schema); DR-022 (book/account/account-at-book vocabulary — context only); DR-021 (Adelaide local timestamps).
